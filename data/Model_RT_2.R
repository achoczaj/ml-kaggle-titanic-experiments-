# Model 2:  Data Science Dojo
# SScore: 0.75598

# RF - Model 2 1.  build an ordinary linear regression (using least squares) to predict the missing values for Age use upper.whisker.Age = 66 as filter for an imput data2.  reuse an ordinary linear regression (using least squares) to predict the missing values for Fare (only 1 value )3.  feed predicted values into dataset4. build a RF predictive model 
...
## str(titanic.full)
equation.Age = "Age ~ Pclass + Sex + SibSp + Embarked"
...
# create formula for the model
## str(titanic.full)
equation.Fare = "Fare ~ Pclass + Sex + SibSp + Parch"
...
write.csv(output.df, file="submission2.csv", row.names = FALSE)