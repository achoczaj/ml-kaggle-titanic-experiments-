...
# # Exclude Survived variable from predicting model
# colnames(titanic.sub_train[, -1])
...
set.seed(42)
indx <- createDataPartition(titanic.train$Survived, # use Survived feature vector as outcomes
...
titanic.train_sub1_70proc <- titanic.train[indx,]
titanic.train_sub2_30proc <- titanic.train[-indx,]
...
# Subset data to features to be used to impute Age
...
# Use carot's dummyVars function to transform all features 
#  to a Full Set of Dummy Variables
dummy.test <- dummyVars(~ ., # transform all non-numeric features (factors) to binary indicators
                        data = titanic.sub_test # use subset data
                        )
# predict binary indicators
titanic.test.dummy <- predict(dummy, titanic.sub_test)
